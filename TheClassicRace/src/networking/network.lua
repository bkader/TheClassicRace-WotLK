...
-- WoW API
local CreateFrame, GetChannelList, GetNumDisplayChannels = CreateFrame, GetChannelList, GetNumDisplayChannels
...
setmetatable(TheClassicRaceNetwork, {__call = function(cls, ...) return cls.new(...) end})
...
function TheClassicRaceNetwork.new(Core, EventBus)
	...
	AceComm:RegisterComm(TheClassicRace.Config.Network.Prefix, function(...) self:HandleAddonMessage(...) end)
	return self
end
...
function TheClassicRaceNetwork:SendObject(event, object, channel, target, prio)
	...
	TheClassicRace:TracePrint("Send Network Event: " .. event .. " Channel: " .. channel .. " Size: " .. string.len(encoded) .. " / " .. string.len(payload))

	AceComm:SendCommMessage(TheClassicRace.Config.Network.Prefix, encoded, channel, target, prio)
end