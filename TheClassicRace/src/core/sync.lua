...
setmetatable(TheClassicRaceSync, {__call = function(cls, ...) return cls.new(...) end})
	...
	for _, offerMode in ipairs(offerModes) do
		...
		if offerMode == "SAME_CLASS_THROTTLED" then
			offers = TheClassicRace.list.filter(self.offers, function(offer)
				return offer.classIndex == classIndex and (offer.lastSync == nil or offer.lastSync < now - OfferSyncThrottle)
			end)
		elseif offerMode == "SAME_CLASS" then
			offers = TheClassicRace.list.filter(self.offers, function(offer) return offer.classIndex == classIndex end)
		elseif offerMode == "THROTTLED" then
			offers = TheClassicRace.list.filter(self.offers, function(offer) return offer.lastSync == nil or offer.lastSync < now - OfferSyncThrottle end)
		else
			offers = self.offers
		end
...
function TheClassicRaceSync:DoSync()
	...
	self.offers = TheClassicRace.list.filter(self.offers, function(offer) return offer.name ~= self.syncPartner.name end)
...
function TheClassicRaceSync:Sync(syncTo, classIndex)
	local batchstr =
		TheClassicRace.Serializer.SerializePlayerInfoBatch(self.DB.factionrealm.leaderboard[classIndex].players)
