...
setmetatable(TheClassicRaceTracker, {__call = function(cls, ...) return cls.new(...) end})
...
function TheClassicRaceTracker:ProcessPlayerInfoBatch(playerInfoBatch, shouldBroadcast, isRebroadcast, classIndex)
	...
	if shouldBroadcast and #batch > 0 and self.DB.profile.options.networking then
		...
		self.Network:SendObject(self.Config.Network.Events.PlayerInfoBatch, {serializedBatch, isRebroadcast, classIndex}, "CHANNEL")
		if IsInGuild() then
			self.Network:SendObject(self.Config.Network.Events.PlayerInfoBatch, {serializedBatch, isRebroadcast, classIndex}, "GUILD")
		end
	end
end

-- ProcessPlayerInfo updates the leaderboard and triggers notifications accordingly
function TheClassicRaceTracker:ProcessPlayerInfo(
	playerInfo)
	...
	TheClassicRace:DebugPrint("[T] ProcessPlayerInfo: [" .. tostring(playerInfo.classIndex) .. "][" .. tostring(playerInfo.class) .. "] " .. playerInfo.name .. " lvl" .. playerInfo.level)
