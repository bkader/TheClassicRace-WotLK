...
setmetatable(TheClassicRaceCore, {__call = function(cls, ...) return cls.new(...) end})
...
function TheClassicRaceCore:ClassIndex(className)
	...
	if TheClassicRace.Config.ClassIndexes[className] ~= nil then
		return TheClassicRace.Config.ClassIndexes[className]
	else
		return TheClassicRace.Config.UnknownClassIndex
	end
end
