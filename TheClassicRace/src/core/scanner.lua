...
setmetatable(TheClassicRaceScanner, {__call = function(cls, ...) return cls.new(...) end})
...
function TheClassicRaceScanner:InitTicker(offset)
	...
	self.Ticker = C_Timer.NewTicker(60 + (offset or 0) + randomOffset, function() self:StartScan() end)
end
